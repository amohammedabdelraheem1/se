...
{
   static uint32_t modeTimer = millis();
  static tMode prevMode = IDEAL;
  if(g_mode != prevMode) {
    modeTimer = millis();
    prevMode = g_mode;
  }
  ...
  {
  case IDEAL:
    if (Door_GetState() == DOOR_CLOSED)
    {
      delay(500); // Wait for door to stabilize
      Door_Lock();
      g_mode = CLOSING;
    }
    else if (KEYPAD_getPressedKey == '*')
    {
      g_mode = USER_PROGRAMMING_MODE;
    }
    else if (CHECK_VALIDATION_GetState() == VALID_STATE)
    {
      if (CARD_GetCardType() == AUTHORISED)
      {
        g_mode = UNIT_PROGRAMMING_MODE;
      }
      else if (CARD_GetCardType() == EMRGANCY)
      {
        Password_Reset();
        g_mode = IDEAL;
      }
      ...
      {
        Serial.println("Invalid Card Type in IDEAL mode");
        g_mode = IDEAL;
      }
    }
    break;
  case CLOSING:
    if (Door_Lock_GetState() == LOCK_CLOSED)
    {
      g_mode = CLOSED;
    }
    else if (Door_Lock_GetState() == LOCK_UNKNOWN && Door_GetState() == DOOR_OPEN) // Timeout for closing
    {
      Door_Unlock();
      g_mode = IDEAL;
    }
    else if ((millis() - modeTimer) >= CLOSING_TIMEOUT_MS)
    {
      Serial.println("Closing timeout, returning to IDEAL mode");
      g_mode = IDEAL;
    }
  ...
  case OPENING:
    ...
    break;
  case OPENED:
    Serial.println("MODE: OPENED");
    break;
  case USER_PROGRAMMING_MODE:
    Serial.println("MODE: USER_PROGRAMMING_MODE");
    break;
  case UNIT_PROGRAMMING_MODE:
    Serial.println("MODE: UNIT_PROGRAMMING_MODE");
    break;
